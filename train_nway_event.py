...
#     for event in events:
#         BIO_trains.append(EnableDataset(subject_list= ['156','185','186','188','189','190', '191', '192', '193', '194'],data_range=(1,45),window_size=500,processed=True,label=i+1,event_label=event))
#         BIO_vals.append(EnableDataset(subject_list= ['156','185','186','188','189','190', '191', '192', '193', '194'],data_range=(45,47),window_size=500,processed=True,label=i+1,event_label=event))
#         BIO_tests.append(EnableDataset(subject_list= ['156','185','186','188','189','190', '191', '192', '193', '194'],data_range=(47,50),window_size=500,processed=True,label=i+1,event_label=event))
        
# BIO_trains.append(EnableDataset(subject_list= ['156','185'],data_range=(1,4),window_size=500,processed=True,label=1))
# BIO_vals.append(EnableDataset(subject_list= ['156'],data_range=(40,46),window_size=500,processed=True,label=1))
# BIO_tests.append(EnableDataset(subject_list= ['156'],data_range=(46,50),window_size=500,processed=True,label=1))
...
weights_list.append(weight_classes(BIO_trains[label_nom[0]]))
...
for i in range(len_class):
    ...
    model = Network(output_dim=numb_class[0])
...
for i in range(len_class):
    # for event in events:
        # loss_history, val_history =train(models[i], criterions[i], optimizers[i], trainloaders[i], valloaders[i], num_epoch, label_no=i+1,event=event)
...
for i in range(len_class):
    # for event in events:
        corr, len_data,_ =evaluate(models[i], testloaders[i],label_no=label_nom[i]+1 )
        corrs.append(corr)
